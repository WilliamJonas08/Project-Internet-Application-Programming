...
router.post(`/`,(req,res)=> {
    ...
    .then(order => {  // Adding the order in our local database   
        ...
        .then(resFromHQ => { //Send the order to HQ 
            ...
            repositoryFcts.syncOrder()
            .then(localId=>{
                localId.forEach(curId => console.log(`Order with id ${curId} was updated`));
                return res.status(201).json({
                    data : resFromHQ.data.data //TODO = addedOrder
                })
            })
            .catch(err => {//Failure
                console.log("Failure modifying order in local Branch db")
                console.log(err)
    ...
    .catch(err => { //Failure when adding/sending order in HQ db
        console.log("Failure when adding the order to Branch db")
        console.log(err)
