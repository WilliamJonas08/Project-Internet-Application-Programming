...
const deleteOrder = function(req){
    ...
    return new Promise(function(resolve, reject) {
        ...
        firestore.collection('orders_hq_processing').doc(id).delete()
        .then(() => {
            console.log('Successfully deleted from the DB')

            firestore.collection('orders_hq_storage').doc(id).update({ status : 'done'})
            .then(updated=>{
                console.log('Order was successfuly archived at the storage');
                resolve(id); 
            })
            .catch(error => {
                console.log('Order with the given ID cannot be archived');
                reject(error.message)
            })
        })
        .catch(error => {
